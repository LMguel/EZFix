...
exports.gerarNotaAutomatica = exports.extrairTextoDaImagem = void 0;
const tesseract_js_1 = __importDefault(require("tesseract.js"));
const sharp_1 = __importDefault(require("sharp"));
const azureVisionService_1 = require("./azureVisionService");
/**
 * Pós-processa o texto para corrigir problemas comuns do OCR em texto manuscrito
 */
const posProcessarTextoManuscrito = (texto) => {
    ...
    for (const [pattern, replacement] of substituicoes) {
        textoCorrigido = textoCorrigido.replace(pattern, replacement);
    }
    ...
    const linhasFiltradas = linhas.filter(linha => {
        ...
        if (/^[^a-zA-ZÀ-ÿ0-9]+$/.test(linhaTrimmed))
            return false;
 ...
 */
const corrigirPalavrasFragmentadas = (texto) => {
...
const extrairTextoDaImagem = async (imageUrl) => {
    // Use a imagem (ou um hash dela) como chave de cache
    const cacheKey = imageUrl.slice(0, 300); // para base64 ou url pequenas
    // Se já estiver processando ou já processou, retorna a mesma Promise
    if (ocrCache.has(cacheKey)) {
        return ocrCache.get(cacheKey);
    }
    // Cria a Promise de OCR e armazena no cache imediatamente
    const ocrPromise = (async () => {
        try {
            ...
            try {
                const azureResult = await (0, azureVisionService_1.extractTextWithAzureRead)(imageUrl.startsWith('data:') || imageUrl.startsWith('http') ? imageUrl : imageUrl);
            ...
            const isLikelyImage = (buf) => {
                if (!buf || buf.length < 8)
                    return false;
                // Use Buffer.subarray instead of deprecated slice signature
                const sig = buf.subarray(0, 8).toString('hex').toLowerCase();
            ...
            let img;
            try {
                img = (0, sharp_1.default)(buffer).grayscale().normalize().sharpen();
            }
            catch (err) {
            ...
            const postMeta = await (0, sharp_1.default)(processed).metadata();
            ...
            if (cols > 1 || rows > 1) {
                ...
                for (let y = 0; y < rows; y++) {
                    for (let x = 0; x < cols; x++) {
                        ...
                        const tile = await (0, sharp_1.default)(processed).extract({ left, top, width: w, height: h }).toBuffer();
                        // guardar também a posição do tile (para mapear bounding boxes)
                        tile.meta = { left, top, width: w, height: h };
                        tiles.push(tile);
                    }
                }
            }
            ...
            const processTile = async (tbuf, index) => {
                ...
                try {
                    ...
                    if (az && az.lines && az.lines.length > 0) {
                        ...
                        return { tileLines: mappedLines, confidence: az.confidence || 80, engine: 'azure' };
                    }
                }
                catch (e) {
                ...
                try {
                    const res = await tesseract_js_1.default.recognize(tbuf, 'por', {
                        logger: (m) => {
                            if (m.status === 'recognizing text')
                                console.log(`Tile ${index + 1} progresso: ${Math.round(m.progress * 100)}%`);
                        }
                    });
                    ...
                    return { text: cleaned, confidence: conf, engine: 'tesseract', tileMeta: tbuf.meta };
                }
                catch (e) {
            ...
            for (let i = 0; i < results.length; i++) {
                ...
                if (r.tileLines && Array.isArray(r.tileLines)) {
                    for (const L of r.tileLines) {
                        allMappedLines.push({ text: L.text || '', left: L.left || 0, top: L.top || 0, confidence: L.confidence || 0, boundingBox: L.boundingBox });
                    }
                }
                else if (r.text) {
                    // dividir texto por linhas e estimar posição a partir do tileMeta center
                    const tileMeta = r.tileMeta || tiles[i].meta || { left: 0, top: 0, width: 0, height: 0 };
            ...
            for (const l of allMappedLines)
                confidences.push({ confidence: l.confidence || 0, length: l.text.length });
            ...
            if (!combinedText || combinedText.trim().length === 0) {
                ...
                return { text: 'Não foi possível extrair texto legível da imagem.', lines: allMappedLines, confidence: weightedConfidence, engine: 'mixed' };
            }
            ...
            if (palavrasDetectadas < 5) {
                return { text: textoLimpo + '\n\n[Nota: Pouco texto foi detectado. Para melhor resultado, tente uma imagem com maior resolução e contraste.]', lines: allMappedLines, confidence: weightedConfidence, engine: 'mixed' };
            }
            return { text: textoLimpo, lines: allMappedLines, confidence: weightedConfidence, engine: 'mixed' };
        }
        catch (error) {
 ...
 */
const gerarNotaAutomatica = (texto) => {
    if (!texto || texto.trim().length === 0)
        return 0;
    // Exemplo simples: pontuar pelo n�mero de palavras
    const palavras = texto.trim().split(/\s+/).length;
    if (palavras < 50)
        return 2.0; // reda��o muito curta
    if (palavras < 150)
        return 5.0; // tamanho m�dio
    if (palavras < 300)
        return 7.0; // bom
