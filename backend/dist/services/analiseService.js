...
exports.analisarTexto = analisarTexto;
const analisarQualidadeOCR = (texto) => {
    const problemas = [];
    ...
    if (confiabilidade < 40)
        nivel = 'baixa';
    else if (confiabilidade < 70)
        nivel = 'media';
    return { nivel, problemas, confiabilidade };
};
const calcularPontuacao = (texto, confiabilidadeOCR) => {
    if (!texto || texto.trim().length === 0)
        return 0;
...
const identificarPontosPositivos = (texto, numPalavras) => {
    const pontos = [];
    if (numPalavras >= 150) {
...
const identificarPontosNegativos = (texto, qualidadeOCR) => {
    const pontos = [];
...
const gerarSugestoes = (texto, qualidadeOCR, numPalavras) => {
    const sugestoes = [];
    if (qualidadeOCR.nivel === 'baixa') {
