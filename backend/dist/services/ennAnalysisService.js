...
exports.analisarEnem = analisarEnem;
exports.formatarTextoComLLM = formatarTextoComLLM;
const openaiService_1 = require("./openaiService");
const analiseService_1 = require("./analiseService");
const promptTemplate = (texto) => `Você é um avaliador de redações segundo os critérios oficiais do ENEM.
...
---FIM TEXTO---
`;
async function analisarEnem(texto) {
    ...
    if (!process.env.OPENAI_API_KEY) {
        const local = (0, analiseService_1.analisarTexto)(texto);
    ...
    const res = await (0, openaiService_1.chamarLLM)(prompt, 1200);
...
async function formatarTextoComLLM(texto) {
    if (!texto || texto.trim().length === 0)
    ...
    if (textoLimpo.includes('assistente') || textoLimpo.includes('JSON') || textoLimpo.includes('textoFormatado')) {
        ...
        for (const linha of linhas) {
            ...
            if (!encontrouTextoReal && linhaTrim === '')
                continue;
...
{"textoFormatado": "texto da redação corrigido e bem formatado", "correcoes": []}`;
    try {
        const { chamarLLM } = await Promise.resolve().then(() => __importStar(require('./openaiService')));
