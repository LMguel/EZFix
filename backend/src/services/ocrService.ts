...
export const extrairTextoDaImagem = async (imageUrl: string): Promise<OCRResult> => {
    ...
    const ocrPromise: Promise<OCRResult> = (async () => {
        try {
            ...
            try {
                ...
                if (azureResult && azureResult.text && azureResult.text.trim().length > 0) {
                    ...
                    if (textoAzureProcessado.split(/\s+/).filter(p=>p.length>0).length >= 5) {
                        return { text: textoAzureProcessado, lines: azureResult.lines, confidence: azureResult.confidence || 80, engine: 'azure' } as OCRResult;
                    }
                }
            } catch (azErr) {
            ...
            const processTile = async (tbuf: Buffer & { meta?: any }, index: number) => {
                ...
                try {
                    ...
                    if (az && az.lines && az.lines.length > 0) {
                        ...
                        return { tileLines: mappedLines, confidence: az.confidence || 80, engine: 'azure' as const } as any;
                    }
                } catch (e) {
                ...
                try {
                    ...
                    return { text: cleaned, confidence: conf, engine: 'tesseract' as const, tileMeta: (tbuf as any).meta } as any;
                } catch (e) {
                    console.warn('Erro OCR tile (tesseract):', e);
                    return { text: '', confidence: 0, engine: 'tesseract' as const };
                }
            };
            ...
            const results: Array<{ text: string; confidence: number; engine: 'azure' | 'tesseract'; tileLines?: any; tileMeta?: any }> = [];
            ...
            let allMappedLines: Array<{ text: string; left: number; top: number; confidence: number; boundingBox?: number[] }> = [];
